...
def get_dataset(source_path,target_path):
    tran_transform = transforms.Compose(
        [
            # transforms.Resize(1024, interpolation=3),
            transforms.ToTensor(),
        ]
    )
...
def calculate_css(source_path,target_path,batch_size=50,num_workers=1):
    dataset = get_dataset(source_path, target_path)
    dataloader = data.DataLoader(
        dataset,
        batch_size=batch_size,
    ...
    total_ssim = 0
    for i, (x, y) in enumerate(dataloader):
        ssim_ = css(x, y, data_range=1, size_average=False)
        total_ssim += ssim_.sum()
    mean = total_ssim / len(dataset)
    return mean
