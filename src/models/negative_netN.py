"""
NEGCUT: source code from https://github.com/WeilunWang/NEGCUT/blob/main/models/negative_generator.py
"""
import torch
import torch.nn as nn
from src.utils.init_util import init_net

def define_N(nce_layers, netN,num_patches,netF_nc, init_type='normal', init_gain=0.02, gpu_ids=[]):
    if netN == 'neg_param':
        net = Negative_Placeholder(nce_layers, num_patches, netF_nc, init_type=init_type, init_gain=init_gain, gpu_ids=gpu_ids)
    elif netN == 'neg_gen':
        net = Negative_Generator(use_conv=True, num_patches=num_patches, nc=netF_nc, init_type=init_type, init_gain=init_gain, gpu_ids=gpu_ids)
    elif netN == 'neg_gen_al' or netN == 'neg_gen_momentum':
        net = Negative_Generator(use_conv=False, num_patches=num_patches, nc=netF_nc, init_type=init_type, init_gain=init_gain, gpu_ids=gpu_ids)
    else:
    ...
    def forward(self, feats, num_patches):
        ...
        for feat_id, feat in enumerate(feats):
            ...
            if torch.cuda.is_available():
                noise = noise.to(feat.device)
            if self.use_conv:
                conv = getattr(self, 'conv_%d' % feat_id)
                feat = conv(feat)
            feat = feat.permute(0, 2, 3, 1).mean(dim=(1, 2)) # 全局平均特征
...
class Negative_Generator_GL(nn.Module):
    def __init__(self, use_conv=False, num_patches=256, nc=256, z_dim=64, init_type='normal', init_gain=0.02, gpu_ids=[]):
        # potential issues: currently, we use the same patch_ids for multiple images in the batch
        super(Negative_Generator, self).__init__()
        self.l2norm = Normalize(2)
