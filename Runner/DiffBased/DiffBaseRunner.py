"""
heavily inspired by:https://github.com/xuekt98/BBDM/blob/main/runners/BaseRunner.py
"""
import datetime 
import pdb
import time
...
from src.utils.save_util import make_save_dirs,make_dir, remove_file
from src.utils.img_util import get_image_grid


import swanlab 
from swanlab.integration.pytorch_lightning import SwanLabLogger
# swanlab login
# my api key: TCtRMRrCgKP9OvKsrUgUq


class DiffusionBaseRunner(ABC):
    def __init__(self,config):
        ...
        # 多卡跑的时候避被多次运行某些函数，确保在主进程运行后就不运行的标志物
        ...
        self.save_config()
        self.writer = None
        if self.config.args.use_swanlab:
            # swanlab_config = {"UPPERFRAMEWORK":"DiffBasedRunner"}
            # swanlab.config.update(vars(self.config))
            # swanlab_logger = swanlab.init(
            #     project="DiffbasedRunner",
            #     name=self.config.args.exp_name,
            #     config = swanlab_config,
            #     mode = self.config.swanlab_mode,
            #     logdir = self.config.args.result_path,
            # )
            # self.writer = swanlab_logger
            swanlab.sync_tensorboard_torch()
            os.makedirs(os.path.join(self.config.args.result_path,"runs"),exist_ok=True)
            self.writer=SummaryWriter(log_dir=os.path.join(self.config.args.result_path,"runs"))
    ...
    def train(self):
        ...
        train_dataset, val_dataset = get_dataset(self.config.data)
        train_sampler = None
        val_sampler = None
        if self.config.training.use_DDP:
                ...
                for train_batch in pbar:
                    ...
                    for i in range(len(self.optimizer)):
                        ...
                        loss = self.loss_fn(net=self.net,
                                            ...
                                            opt_idx=i,
                                            stage='train',write=self.config.args.use_swanlab)
    ...
    @torch.no_grad()
    def validate_step(self,val_batch,epoch,step):
        self.apply_ema()
        self.net.eval()
        loss = self.loss_fn(
            net=self.net,
            ...
            epoch=epoch,
            step=step,
            opt_idx=0, # optimizer index 0是第一个optimizer的参数
            stage='val_step'
        )
        if len(self.optimizer)>1:
            # 如果有GAN的判别器部分就再加一个loss
            loss = self.loss_fn(
            net=self.net,
            batch=val_batch,
            epoch=epoch,
            step=step,
            opt_idx=1, # optimizer index 0是第一个optimizer的参数
            stage='val_step'
            )
        self.restore_ema()
    ...
    def initialize_model_optimizer_scheduler(self, config, is_test=False):
        ...
        """
        net = self.initialize_model(config) # need to overwrite
        optimizer, scheduler = None, None
        if not is_test:
            optimizer, scheduler = self.initialize_optimizer_scheduler(net, config) # need to overwrite
