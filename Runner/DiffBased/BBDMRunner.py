...
from Runner.DiffBased.DiffBaseRunner import DiffusionBaseRunner
from src.diffusion.BrownianBridgeModel import BrownianBridgeModel
from dataset import get_dataset,get_optimizer
from src.utils.save_util import make_dir
from src.utils.img_util import get_image_grid,save_single_image
from src.utils.init_util import weights_init


@Registers.runners.register_with_name("BBDM_Runner")
class BBDMRunner(DiffusionBaseRunner):
    ...
    def initialize_optimizer_scheduler(self, net,config,is_test=False):
        ...
        optimizer = get_optimizer(config.model.BB.optimizer,parameters=net.parameters())
        if config.model.BB.lr_scheduler.type=='linear':
            schedulerG = torch.optim.lr_scheduler.LambdaLR(optimizer=optimizer,lr_lambda=lambda_rule)
        else:
            from types import SimpleNamespace
            del config.model.BB.lr_scheduler.type
