...
from src.losses import AdaptiveSupervisedPatchNCELoss,GANLoss,PatchNCELoss,MarginalNCE
...
@Registers.runners.register_with_name("MCLCUT_Runner")
class MCLCUTRunner(GANBaseRunner):
    def __init__(self,config):
        ...
        if self.config.args.use_swanlab:
            ...
            swanlab_logger = SwanLabLogger(
                ...
                mode = self.config.args.swanlab_mode,
    ...
    def train(self):
        ...
        for nce_layer in self.nce_layers:
            self.criterionNCE.append(MarginalNCE(self.opt).to(self.device))
        # 最优传输的引入大大减轻了多对一特征匹配的约束，同时在条件输入和样本之间建立了准确的语义对应
        first_step = False
